...
export interface Tree {
  ...
  push(ent: TreeEnt[]): Promise<TreeEnt[] | void>;
}
...
export class TreeFs {
  dump(fs: Vfs, mount = "/"): TreeEnt[] {
    ...
    const go = (n: VNode, p: string): void => {
      ...
      if (n instanceof Reg) { e.data = n.data; e.sum = n.sum ??= fileSum(n.data); }
  ...
  private chk(e: TreeEnt): void {
    ...
    if ((e.k === "f" && (!(e.data instanceof Uint8Array) || e.sum !== undefined && typeof e.sum !== "string")) || (e.k === "l" && typeof e.to !== "string")) throw new Error("bad host tree payload");
  }
}
